...
Этот проект — часть набора: **Платформа** (SUE_system, web-desktop), **Контроль СИЗ** (siz-control) и Учёт ПУ. Commit и push после правок делаются по умолчанию.

**Важно:** это **Render-версия** (`github.com/ashinoff/Uchet_PU`). Есть параллельный репозиторий **`Uchet_PU_amvera`** — деплой на Amvera + интеграция с платформой (Keycloak SSO), там же приложение зовётся «Светлячок». Правки бизнес-логики карточки ПУ обычно нужно синхронизировать в ОБА репо (код почти идентичен; в amvera сверху — SSO/бейдж, которые тут отсутствуют).

## Журнал изменений
- **2026-07-04** — `restore_backup` переписан: справочники матчатся по натуральным
  ключам (PUTypeReference→pattern, Material/VA/TT→name, TTR_RES→code, TTR_ESK→
  комбинация), новые записи создаются БЕЗ id из бэкапа (id даёт БД, `flush`),
  строится ремап `old_id→new_id` и применяется ко всем FK pu_items (в т.ч.
  добавлены ранее пропущенные `ttr_ou/ol/or/tt_id`, `ttr_esk_id`). Дозаполняет
  пустые `email`/`connection_type` в существующих (не перезаписывая непустое);
  ответ `restored{created,updated}` по каждой сущности (фронт-alert обновлён).
  Синхронно с `Uchet_PU_amvera` (`eefdd74`). Коммит `79d82ab`.
- **2026-07-10** — Карточка ПУ, ОКС + способ СМР «Подрядчик»: ТТР и материалы
  назначаются только при хоз способе. Флаг `isOksContractor` скрывает блоки ТТР
  (`ttr_ou/ol/or_id` + авто У-27) и «Материалы»; при выборе «Подрядчик» —
  `ttr_*_id`→null и `setMaterials([])` (бэкенд по сброшенным ТТР сам удаляет
  `PUMaterial`); валидация: правило «ВА→ttr_or_id» не применяется для
  ОКС+Подрядчик, а при отправке на согласование обязателен `contractor_id`.
  Только `frontend/src/App.jsx`, бэкенд не менялся. Коммит `42ed8f9`.
  Синхронный фикс в `Uchet_PU_amvera` — `bf6d5b3`.
